...
(async()=>{
  ...
  try{
    ...
    await page.waitForFunction(()=>window.V21TideObservatoryArt?.metrics.materials>0&&window.V21TideObservatoryArt?.metrics.rooms>0,null,{timeout:30000});
    await page.waitForTimeout(1200);
    const result=await page.evaluate(()=>({rooms:game.journey.rooms.length,key:SeamlessFloor.key,checkpoint:JSON.parse(localStorage.getItem(SeamlessFloor.key)),art:!!window.V21TideObservatoryArt,build:window.V20Build?.installed,imageRendering:getComputedStyle(game.canvas).imageRendering}));
    ...
    if(result.imageRendering!=='auto')throw Error('Observatory is still using pixelated browser scaling');
