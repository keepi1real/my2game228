...
Game.prototype.playerAttack = function () {
  const before = this.enemies.filter((e) => e.alive).length;
  const hpBefore = this.enemies.reduce((s, e) => s + (e.alive ? e.hp : 0), 0);
  ...
  const hpAfter = this.enemies.reduce((s, e) => s + (e.alive ? e.hp : 0), 0);
  if (this.hero.attack.type === 'melee' && hpAfter >= hpBefore && before > 0) Sound.play('miss', { vol: .6 });
};
...
Game.prototype.playerDie = function (killer) {
  ...
  if (was === 'run') { Sound.stopMusic(1.2); Sound.play('death_hero', { throttle: 0 }); }
};
...
Game.prototype.killEnemy = function (e) {
  ...
  Sound.play(boss ? 'boss_roar' : big ? 'death_big' : 'death_small', { throttle: boss ? 0 : .05, vol: boss ? 1 : .8 });
};

// ---------- Умения и рывок ----------
const sfxBaseSkill = Game.prototype.useSkill;
Game.prototype.useSkill = function (i) {
  const ready = this.player && this.player.skillCds[i] <= 0;
  sfxBaseSkill.call(this, i);
  if (ready) Sound.play('skill_' + this.hero.skills[i], { throttle: .05 });
};
...
Game.prototype.openChest = function (c) {
  ...
  if (!was) Sound.play('chest_open', { throttle: 0 });
};
...
if (Game.prototype.chooseRelic) {
  ...
  Game.prototype.chooseRelic = function (id) {
    ...
    Sound.play('relic_pickup', { throttle: 0 });
    return r;
  };
}
...
if (Game.prototype.descendSeamlessFloor) {
  ...
  Game.prototype.descendSeamlessFloor = function () {
    ...
    if (ok) Sound.play('zone_enter', { throttle: 0 });
    return ok;
  };
}
...
if (typeof AdventureProgress !== 'undefined' && AdventureProgress.learn) {
  ...
  AdventureProgress.learn = function (g, id) {
    ...
    if (ok) Sound.play('talent_learn', { throttle: 0 });
    return ok;
  };
}
...
Game.prototype.endRun = function (victory) {
  ...
  if (victory) Sound.play('level_up', { throttle: 0 });
};
...
UI.prototype.showPause = function () {
  ...
  buttons.insertAdjacentHTML('beforebegin', this.volumeHtml());
  this.bindVolumes();
};
