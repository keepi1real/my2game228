...
function layout(w=1024,h=640,saved=false){
  ...
  function paint(c,menu,time){
    ...
    for(const b of L.boxes){
      ...
      if(b.hero){const sheet=ActorMotion.sheetImage(b.hero),img=sheet||artImage('portrait','heroes',b.hero);if(ready(img)){c.save();c.beginPath();c.rect(b.x+u,b.y+u,b.w-2*u,b.h-2*u);c.clip();if(sheet){const d=ActorMotion.sheetLayout(b.hero),k=Math.min(b.h*.86/d.figureH,b.w*.88/90);c.imageSmoothingEnabled=false;c.drawImage(sheet,0,0,192,192,b.x+b.w/2-d.footX*k,b.y+b.h*.94-d.footY*k,192*k,192*k);}else{const d=portraitDef('heroes',b.hero),sw=img.width/(d.zoom||2),sh=sw*(b.h/b.w),sx=img.width*(d.focusX||.5)-sw/2,sy=Math.max(0,img.height*(d.focusY||.2)-sh*.38);c.drawImage(img,sx,sy,sw,sh,b.x,b.y,b.w,b.h);}c.restore();}if(chosen){c.strokeStyle='#efd69a';c.lineWidth=2*u;c.strokeRect(b.x,b.y,b.w,b.h);c.fillStyle='#eed497';c.fillRect(b.x+b.w/2-3*u,b.y+b.h-2*u,6*u,4*u);}}
